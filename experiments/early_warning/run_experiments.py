...
# -*- coding: utf-8 -*-
"""
TCR-GIN/experiments/early_warning/run_experiments.py

Experiment launcher for the early-warning training pipeline.

Function
--------
This script reads a campaign-level YAML configuration file, expands it into
individual experiment tasks, launches training jobs one by one, stores logs and
artifacts, performs temporary-data cleanup, and finally generates a summary
Excel report.

Supported experiment modes
--------------------------
1. Standard batch experiments
2. Multi-source experiments based on temporary symlinked directories
3. Dynamic-source experiments based on copied subsets from a master dataset root
4. Grid search via `param_grid`
5. Single-variable ablation / sensitivity analysis via `ablation_grid` or
   `sensitivity_grid`

Inputs
------
- A YAML config file passed through `--config`
- Optional source datasets referenced by that config
- `train_piss.py` in the same directory

Outputs
-------
- Per-run log files under the configured output directory
- Model checkpoints under the configured model directory
- A final Excel summary report:
    summary_report_<campaign_name>.xlsx

Usage
-----
Example:
    python experiments/early_warning/run_experiments.py \
        --config experiments/early_warning/configs/train-multisource-353.yaml
"""
...
TRAIN_SCRIPT = str(CURRENT_SCRIPT_DIR / "train_piss.py")


# ==============================================================================
# Multi-source temporary directory setup / cleanup
# ==============================================================================
def setup_symlinked_dirs(config):
    ...
    print(f"[*] Preparing multi-source aggregated directory: {temp_dir.resolve()}")
...
def cleanup_symlinked_dirs(config):
    ...
    temp_dir = Path(config['temp_data_dir'])
    if temp_dir.exists():
        print(f"[*] Cleaning temporary aggregated directory: {temp_dir.resolve()}")
        shutil.rmtree(temp_dir)


# ==============================================================================
# Dynamic-source temporary directory setup / cleanup
# ==============================================================================
def prepare_dynamic_source_dirs(instance_config, master_root, templates):
    """
    Build a temporary local dataset for a dynamic-source task by copying files
    from the master dataset root according to the instance definition.
    """
    ...
    if local_temp_root.exists() and any((local_temp_root / 'train').iterdir()):
        print(f"[+] Existing dynamic dataset '{mix_id}' found. Skipping copy.")
        return {
    ...
    print(f"[*] Creating dynamic dataset for '{mix_id}' at: {local_temp_root.resolve()}")
    ...
    if not files_to_copy:
        print(
            ...
            f"Please check 'master_data_root' and the dataset directory structure."
        )
    else:
        print(f"[*] Copying {len(files_to_copy)} files from the master storage...")
        for src, dest in tqdm(files_to_copy, desc=f"Copying data ({mix_id})"):
            shutil.copy(src, dest)

    print(f"[+] Dynamic dataset '{mix_id}' created successfully.")
...
# ==============================================================================
# Log parsing and summary report generation
# ==============================================================================
def summarize(directory, campaign_name, all_param_keys):
    """
    Scan experiment logs, aggregate results by scenario, and generate a compact
    multi-sheet Excel summary report.
    """
    print(f"\n[*] Scanning logs and building summary for campaign '{campaign_name}': {directory}")
    ...
    if not log_files:
        print("[!] No valid log files (*.log) were found.")
        return
    ...
    if not all_results:
        print("[!] Log files were parsed, but no valid results were extracted.")
        return
    ...
    try:
        with pd.ExcelWriter(output_filename, engine='openpyxl') as writer:
            print("\n[*] Aggregating results by scenario and writing Excel sheets...")
            ...
            for sheet_name, group_df in df.groupby('sheet_name'):
                ...
                if not metric_cols:
                    print(f"  - [Warning] No expected metrics found in '{sheet_name}'. Skipping.")
                    continue

                param_cols = sorted(
                    [p.split('.')[-1] for p in all_param_keys if p.split('.')[-1] in group_df.columns]
                )
        ...
        print(f"\n[+] Summary report saved successfully: {output_filename.resolve()}")
    except Exception as e:
        print(f"\n[!] Failed to save the Excel summary report: {e}")


def parse_log_content(filepath):
    """
    Extract test metrics from a log file.

    Note:
    This parser matches the current training-log format and keeps the original
    behavior unchanged.
    """
    results_list = []
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            ...
            test_matches = re.findall(
                r"Test Results - RMSE: ([\d.eE+-]+), MAE: ([\d.eE+-]+), R²: ([\d.eE+-]+)",
                content
            )
...
def parse_log_info(filepath, base_dir, all_param_keys):
    """Infer sheet names and parameter values from the log file path and name."""
    info = {}
    try:
...
# ==============================================================================
def main():
    parser = argparse.ArgumentParser(description="Unified experiment launcher")
    parser.add_argument('--config', type=str, required=True, help="Path to the experiment YAML config file.")
    args = parser.parse_args()

    os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "max_split_size_mb:512"
    print("[*] Set PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:512 for improved GPU memory management.")
    ...
    print("[+] Pre-launch cleanup finished.")
    ...
    print(f"====== Starting experiment campaign: {campaign_name} ======")
                    ...
                    for key in ['train_source_dirs', 'val_source_dirs', 'test_source_dirs']:
                        if key in templates:
                            multi_source_config_instance[key] = [
                                path_template.format(**format_vars) for path_template in templates[key]
                            ]
    ...
    print(f"\n[+] Task generation complete: {len(all_tasks)} independent experiment tasks.")
    print(f"[+] Models will be saved to: {main_model_dir.resolve()}")
    print(f"[+] Logs and outputs will be saved to: {main_output_dir.resolve()}")
    print("\n" + "=" * 80)
    ...
    for i, task in enumerate(all_tasks):
        ...
        try:
            ...
            if return_code != 0:
                ...
                print(f"[+] Task {sub_experiment_name} finished successfully.")
        except Exception as e:
            print(f"\n[!] A critical error occurred while launching or running {sub_experiment_name}: {e}")
    ...
    print("\n" + "=" * 80)
    print(f"[+] Experiment campaign '{campaign_name}' completed.")

    print("\n[*] Running post-execution cleanup...")
    if completed_temp_dirs:
        print("  - Cleaning temporary dataset directories...")
        for temp_dir_str in completed_temp_dirs:
            cleanup_dynamic_source_dirs(temp_dir_str)

    cache_dir = Path('cache')
    if cache_dir.exists():
        print(f"  - Removing cache directory: {cache_dir.resolve()}")
        shutil.rmtree(cache_dir)

    print("[+] Post-execution cleanup finished.")
    print("=" * 80)
