...
# -*- coding: utf-8 -*-
"""
TCR-GIN/experiments/baseline_comparison/ablation/test_ablation.py
...
Example:
    ...
    python experiments/baseline_comparison/ablation/test_ablation.py \
        --config experiments/baseline_comparison/ablation/configs/test_ablation_REDDIT.yaml
"""
...
def collect_ablation_dataset_paths(config):
    all_paths = set()
...
def manage_test_data(config):
    ...
    dataset_rel_paths = collect_ablation_dataset_paths(config)

    temp_dir = os.path.join(PROJECT_ROOT, f"temp_data_cache_ablation_{int(time.time())}")
...
# [REVISED] Expanded key mapping to catch 'modeluseresidual' etc.
def parse_exp_name_to_params(exp_name):
    params = {}
    key_mapping = {
        'modelusevirtualnode': 'model.use_virtual_node',
        'pissconsistencylambda': 'piss.consistency_lambda',
        'modelactivationfn': 'model.activation_fn',
        'modeljktype': 'model.jk_type',
        'modeluseresidual': 'model.use_residual',
        'modelnumlayers': 'model.num_layers',
...
def run_ablation_exp(task_config):
    global_settings = task_config['global_settings']
    model_source = task_config['model_source']
    test_dataset = task_config['test_dataset']

    model_base_dir = os.path.join(PROJECT_ROOT, global_settings['models_root_dir'], model_source['path'])
    model_dir = model_base_dir

    # fallback: if current directory is parent, look for exp_* inside it
    ...
    print(f"\n- Testing Ablation Config: {exp_name}")
...
def generate_ablation_tasks(config, base_config):
    tasks = []
    experiments = config.get('experiments', [])
    ...
    print(f"  - Generated {len(tasks)} total ablation testing tasks.")
    return tasks


def main():
    parser = argparse.ArgumentParser(description="GNN Ablation Study Testing Framework")
    parser.add_argument(
        '--config',
        type=str,
        required=True,
    ...
    print("=" * 60 + "\nGNN Ablation Study Testing Framework\n" + "=" * 60)
    ...
    with manage_test_data(config) as temp_data_root:
        ...
        all_tasks = generate_ablation_tasks(config, config['base_config'])
        if not all_tasks:
            print("\n[INFO] No valid tasks were generated.")
        else:
            all_results = [
                result for task in tqdm(all_tasks, desc="Executing ablation tasks")
                if (result := run_ablation_exp(task))
            ]
    ...
    print(f"\n[SUCCESS] All jobs completed. Ablation study results saved to:\n{output_path}")
    print("\n" + "=" * 60 + "\nTest Run Finished\n" + "=" * 60)
